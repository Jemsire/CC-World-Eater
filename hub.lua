...
-- Create a copy of shared folder in each directory
for _, filename in pairs(fs.list('/disk/shared_files')) do
    if(fs.exists('/disk/hub_files/' .. filename)) then
        fs.delete('/disk/hub_files/' .. filename)
    end
    if(fs.exists('/disk/turtle_files/' .. filename)) then
        fs.delete('/disk/turtle_files/' .. filename)
    end

    fs.copy('/disk/shared_files/' .. filename, '/disk/hub_files/' .. filename)
    fs.copy('/disk/shared_files/' .. filename, '/disk/turtle_files/' .. filename)
end

-- Grab all hub files from disk and rename them to local storage
